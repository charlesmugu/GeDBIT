...
public class MSMSMetric implements Metric {
    ...
    public double getDistance(SpectraWithPrecursorMass v1, SpectraWithPrecursorMass v2) {
        ...
        if (Math.abs(Math.abs(cos) - 1) < COS_THRESHOLD) { // precision
            ...
            System.out.println("got cosine > 1, cosine=" + cos + ", :" + v1.toString() + ", v2:"
                    + v2.toString() + "Quitting.");
            System.exit(0);
        }
        // }
         ...
         * System.out.println("mscosDist = " + mscosdist + ", Cosine = " + cos + ", MassDiff = " +
         * massDiff); System.out.println("Total Dist = " + dist); System.out.println();
         */
        return dist;
        ...
        // return (1 - cos);
        /*
         * //extended jaccard double[] one = v1.getData(); double[] two = v2.getData(); double spc =
         * getInnerProduct(one, two); double dist = 1 - (spc / (getMagnitude(one) +
         * getMagnitude(two) - spc)); return dist;
         */
        // getCosine(,v2));//weijia
    }
    ...
    /**
     * For experiment purpose only Method to compute inner product, cosine value and distance of v2
     * {@link VectorData} objects by treating them as binary vectors in a multidimensional space.The
     * Value will be stored in a {@link double[]} with the correspoding order.
     * 
     * @param -
     *        {@link VectorData}
     * @param v2 -
     ...
     */
    /*
     * public double[] getAll(DataVector one, DataVector two) { double[] v1 = one.getData();
     * double[] v2 = two.getData(); double[] result = new double[3]; result[0] = getInnerProduct(v1,
     * v2); result[1] = result[0] / (getMagnitude(v1) * getMagnitude(v2)); result[2] =
     * Math.(result[1]); return result; }
     */
    ...
    /**
     * Convert information of the instance into a {@link String} that contains the number of
    ...
    public String printDistance(SpectraWithPrecursorMass k1, SpectraWithPrecursorMass k2) {
        ...
        // outStr.append("k2: " + k2 + "\n");
        outStr
                .append("MSCOSDIST = " + frm.format(mscosdist) + ", MASS_DIFF_TERM = "
                        + frm.format(massDiffTerm) + " (abs mass diff = " + frm.format(absMassDiff)
                        + ")\n");
