...
public class MSMetric implements Metric {
	...
	/** default constructor - is a temporary solution providing default values for
	...
	/**
	 * @param v1 the {@link Object} over which the keys are defined.
	 * @param v2 the other {@link Object} over which the keys are defined.
	 */
	...
	/**
	 * @param v1 the {@link MSKeyObject} over which the keys are defined.
	 * @param v2 the other {@link MSKeyObject} over which the keys are defined.
	 */
	public double getDistance(Spectra v1, Spectra v2) {
		...
		System.out.println("Dist = "  + Math.acos(cos) + ",Cosine = " + cos);
		System.out.println();
		System.out.println();
		*/
		...
		double[] one = v1.getData();
		double[] two = v2.getData();
		
		double spc = getInnerProduct(one, two);
		double dist = 1 - (spc / (getMagnitude(one) + getMagnitude(two) - spc));
		return dist;*/
		//getCosine(,v2));//weijia
	}
	...
	private double getCosine(Spectra one, Spectra two) {
		...
		// smriti - 30sep
		/*
		double numer = getInnerProduct(v1, v2) ;
		double denom = getMagnitude(v1) * getMagnitude(v2);
		
		double denom = getMagnitude(v1) * getMagnitude(v1);
		double d2 = getMagnitude(v2) * getMagnitude(v2);
		denom = (denom + d2) / 2;
	...
	/** This is the method that replaces calls to super.getInnerProduct()
	  * Compute FuzzyInnerProduct withing a certain tolerance.
	  */
	...
	* Method to compute inner product, cosine value and distance of v2
	* {@link VectorData} objects by treating them as binary vectors in a
	* multidimensional space.The Value will be stored in a {@link double[]}
	* with the correspoding order.
	* @param  - {@link VectorData}
	* @param v2 - {@link vectorData}
	* @return {@link double []}
	**/
	/*public double[] getAll(DataVector one, DataVector two) {
		double[] v1 = one.getData();
		double[] v2 = two.getData();
		double[] result = new double[3];
		result[0] = getInnerProduct(v1, v2);
		result[1] = result[0] / (getMagnitude(v1) * getMagnitude(v2));
		result[2] = Math.(result[1]);
		
		return result;
	}*/

	/**
	* Computes the magnitude of the {@link VectorData} object by
	* treating it as a binary vector
	* @param {@link VectorData}
	**/
	...
	/**
	 *Convert information of the instance into a {@link String} that
	*contains the number of dimension and meaning for each dimension
	**/
	/* public String toString()
	{
			String result="incomplete implementation";
		return result;
	 }*/

	//-- Changing this for mSQL serializability reqmt - dont need custom functions here anyway
	//legacy
