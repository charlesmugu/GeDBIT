...
public class Histogram {
	...
	public BinInfo addPoint(double value) {
	    if ((value - (center - width / 2) < -Delta)
		    || (value - (center + width / 2) >= Delta))
     ...
     * 
     * @param start
     *            the starting boundary of bins, bins can be left to this
     *            starting point.
     * @param width
     ...
     * @param last
     *            exclusive
     * @return a List of BinInfo, non-empty, non-overlaping. sorted asendingly
     *         by the center value.
     */
    public static ArrayList<BinInfo> completeOneDHistogram(double start,
	    double width, double[] dist, int first, int last) {
	TreeMap<Integer, BinInfo> map = new TreeMap<Integer, BinInfo>();
	for (int i = first; i < last; i++) {
	    double temp = (dist[i] - start) / width;
	    int offset = (Math.abs(temp - Math.ceil(temp)) <= Delta) ? (int) Math
		    .ceil(temp) : (int) Math.floor(temp);
	    if (map.containsKey(offset)) {
		map.put(offset, map.get(offset).addPoint(dist[i]));
	    } else
		map.put(offset, (new BinInfo(start + (offset + 0.5) * width,
			width)).addPoint(dist[i]));
	}
     ...
     * @param dist
     *            the array of distances to compute histogram
     * @return a 2-d double array of 2 rows. the first row is the upper bound of
     *         each bin, the second row is the corresponding bin size
     */
    public static double[][] oneDHistogram(double start, double width,
	    double[] dist) {
    ...
    /**
     * Given the 2-d (x-y) values, ranges and number of bins, out put the
     * (non-zero) number of occurances of each 2-d value. A large 2-d matrix
     * will be used internally, therefore, might not work if numbers of bins are
     * too large.
     * 
     * @param xMin
     *            minimum value of x
     ...
     *            maximum value of x
     * @param xSize
     *            number of bins for x, or more accurately, how many parts the x
     *            value range will be divided into
     * @param xValue
     *            values of x
     * @param yMin
     *            minimum value of y
     ...
     *            maximum value of y
     * @param ySize
     *            number of bins for y, or more accurately, how many parts the y
     *            value range will be divided into
     * @param yValue
     *            values of y
     * @return the occurances //3-d vector, first row are x-values, second
     *         y-values, third row consists of occurances
     */
    public static int[][] TwoDHist(double xMin, double xMax, int xSize,
	    double[] xValue, double yMin, double yMax, int ySize,
	    double[] yValue) {
	// check argument
	if (xValue.length != yValue.length)
	    throw new IllegalArgumentException(
		    "x, y values of different length!");

	final double xBinWidth = (xMax - xMin) / xSize;
	final double yBinWidth = (yMax - yMin) / ySize;

	int[][] counter = new int[xSize + 1][ySize + 1]; // allocate 1 more row
							 // and column to
							 // allow the maximum
							 // value
	...
	/*
	 * int length =0; for (int i=0; i< xSize+1; i++) for (int j=0;
	 * j<ySize+1; j++) if (counter[i][j] > 0) length ++; //out put double
	 * [][] result = new double [3][length]; length =0; for (int i=0; i<
	 * xSize; i++) for (int j=0; j<ySize; j++) if (counter[i][j] >0) {
	 * result[0][length] = xMin +i*xBinWidth; result[1][length] = yMin
	 * +j*yBinWidth; result[2][length] = counter[i][j]; length ++; }
	 */
     ...
     * distances to each pivot. the file should be a text file. The first part,
     * separted by whitespace, of the first line should be a number, the number
     * of pivots. then each following line is the description of a pivot, in the
     * order. then an empty line. then, the first part of next line should be a
     * number, number of points then each following line should be the
     * coordinates of a point, separted by comma. in the command line, the first
     * parameter should be the file name, then a series of numbers or chars, x0,
     * x1, ..., xn, corresponding to dimensions d0, d1, ..., dn, less number of
     * number/chars can be provided, they will start at dim 0, dims not
     * specified will be ignored. if xi is '-', then dimension i is ignored, if
     * xi is '.', then the histogram on dimension i will be computed, if xi is a
     * number, then the value on dimension i should be fixed as that number,
     * other values are ignored. min, max values of each dimension to compute
     * histogram will be reported. if only one dimension to compute histogram,
     * the histogram will be out put in one line, if two, then a matrix, if
     * more, then each non-zero tuple will be output. the out put is to the
     * screen.
     * 
     * @param args
     *            the first should be the file name, then a series of numbers or
     *            chars, x0, x1, ..., xn, corresponding to dimensions d0, d1,
     *            ..., dn.
     */
	...
	boolean valid = false; // whether current line is consistent with the
			       // fixed values
	...
	for (int j = 0; j < dataSize; j++) {
	    ...
	    for (int i = 0; i < pivotNumber; i++)
		if (isFixed[i]
			&& ((int) Double.parseDouble(lineSegment[i]) != fixedValue[i])) {
		    valid = false;
		    break;
		}
	    ...
	    for (int i = 0; i < pivotNumber; i++)
		if (toCompute[i])
		    keyBuffer.append((char) (int) Double
			    .parseDouble(lineSegment[i]));
	...
	for (int i = 0; i < pivotNumber; i++)
	    if (toCompute[i])
		histDim++;
	int[] mapping = new int[histDim]; // mapping[i] the dim number of the
					  // ith dim to compute
					  // histogram
	int j = 0;
	for (int i = 0; i < pivotNumber; i++)
	...
	System.out.println("map size  = " + map.size() + ",   dataset size = "
		+ sum + ",  pivot number = " + pivotNumber);
	// var /= map.size();

	System.out.print("#[min, max] values of each dim of histogram: ");
	for (int i = 0; i < histDim; i++)
	    System.out.print("pivot " + pivot[mapping[i]] + ": [" + min[i]
		    + ", " + max[i] + " ], ");
	System.out.println();
	...
	if (histDim == 2) {
		...
		System.out
			.println("#Histogram: "
				+ " std. dev. = "
				+ Math.sqrt(var
					/ (histogram2D.length * histogram2D[0].length)
					- Math.pow(
						sum
							/ (histogram2D.length * histogram2D[0].length),
						2))
				+ ", std. dev. of non-zero buckets = "
				+ Math.sqrt(var / map.size()
					- Math.pow(sum / map.size(), 2))
	...
	while (p.hasNext()) {
	    ...
	    key = (String) entry.getKey();
	    if (histDim == 1) {
		histogram1D[(int) key.charAt(0) - min[0]] = ((BigInteger) entry
			.getValue()).intValue();
	    } else if (histDim == 2) {
		histogram2D[(int) key.charAt(0) - min[0]][(int) key.charAt(1)
			- min[1]] = ((BigInteger) entry.getValue()).intValue();
	    } else {
		System.out.print("(");
		for (int i = 0; i < histDim; i++)
	...
	if (histDim == 2) {
		...
		/*
		 * System.out.println("# for gnuplot"); if ( (min[0] > 0 ) ||
		 * (min[1] >0) )
		 * System.out.println("# warning! min is not 0!!"); for (int
		 * i=0; i<histogram2D.length; i++) { for ( j=0; j<
		 * histogram2D[0].length; j++) if (histogram2D[i][j] >0)
		 ...
		 * System.out.println("# for gnuplot"); if ( min[0] > 0 )
		 * System.out.println("# warning! min is not 0!!"); for (int
		 * i=0; i<histogram1D.length; i++) System.out.println( (i +
		 * min[0]) + " " + histogram1D[i] );
		 */
	    }
	}
     ...
     * distances to each pivot. The file format is the same as those required by
     * other methods. the file should be a text file. The first part, separted
     * by whitespace, of the first line should be a number, the number of
     * pivots. then each following line is the description of a pivot, in the
     * order. then an empty line. then, the first part of next line should be a
     * number, number of points then each following line should be the
     * coordinates of a point, separted by comma. in the command line, the first
     * parameter should be the file name, the second and third should be
     * sequential id of dimensions to compute histogram, start from 0. the
     * output is the 2-d matrix histogram of the two designated dimensions the
     * out put is to the screen.
     * 
     * @param args
     *            the first should be the file name, the second and third should
     *            be sequential id of dimensions to compute histogram, start
     *            from 0.
     */
    ...
    /**
     * Read a file consisting of coordiantes of points in pivot-space, i.e.
     * distances to each pivot. This distance values are real number,
     * double/float the file should be a text file. The first part, separted by
     * whitespace, of the first line should be a number, the number of pivots.
     * then each following line is the description of a pivot, in the order.
     * then an empty line. then, the first part of next line should be a number,
     * number of points then each following line should be the coordinates of a
     * point, separted by comma. in the command line, the first parameter should
     * be the file name, then a series of sets of parameters for each dimension
     * sequentially, if not enough sets of parameters are provided, later
     * dimensions are ignored. a set of parameters for a dimension can be of one
     * of the 3 formats: format 1: two double values separated by white space,
     * means this dimension is fixed, in the range provided by the two doubles,
     * first should not be larger than the second, left end inclusive, right end
     * exclusive. format 2: "-", indicating this dimension is ignored format 3:
     * "." followed by a number, indicating that this dimension is subject to
     * histogram computation, and the number is the width of the bin. min, max
     * values of each dimension to compute histogram will be reported. if only
     * one dimension to compute histogram, the histogram will be out put in one
     * line, if two, then a matrix, if more, then each non-zero tuple will be
     * output. the out put is to the screen.
     * 
     * @param args
     *            the first should be the file name, then width of bin. then a
     *            series of numbers or chars, x0, x1, ..., xn, corresponding to
     *            dimensions d0, d1, ..., dn.
     */
	...
	// process parameters
	double[] lowerBound = new double[pivotNumber]; // for fixed dims, stores
						       // lower(inclusive) /
						       // upper(exclusive) bound
						       // of the bin,
	double[] upperBound = new double[pivotNumber]; // for dims to computes,
						       // stores min / max
						       // value.
	...
	while ((parameter < args.length) && (pivotInvolved < pivotNumber)) {
	    if (args[parameter].equalsIgnoreCase(".")) // this is a dimension to
						       // compute histogram
	    {
	...
	int[] fixedMapping = new int[fixedDim]; // fixedMapping[i] is the dim id
						// of ith dim that is
						// fixed
	int[] histMapping = new int[histDim]; // histMapping[i] is the dim id of
					      // ith dim to
					      // compute histogram
	...
	// read (only the necessary part of valid) data into arrays, get
	// lower/upper bounds of the
	// dimensions to compute histogram
	double[][] distance = new double[histDim][dataSize];
	int counter = 0; // the counter on valid points
	boolean valid = false; // whether current point is consistent with the
			       // fixed values
	double temp = 0;
	for (int i = 0; i < dataSize; i++) {
		...
		if ((temp < lowerBound[fixedMapping[j]])
			|| (temp >= upperBound[fixedMapping[j]])) {
		    valid = false;
		    break;
		}
	    }
		...
		distance[j][counter] = Double
			.parseDouble(lineSegment[histMapping[j]]);
	...
	TreeMap<String, BigInteger> map = new TreeMap<String, BigInteger>();
	TreeMap<Integer, BigInteger>[] histMap = new TreeMap[histDim]; // a map
								       // for
								       // each
								       // dimension
								       // to
	...
	for (int i = 0; i < counter; i++) {
		...
		if (histMap[j].containsKey(offsetKey))
		    histMap[j].put(offsetKey, ((BigInteger) histMap[j]
			    .get(offsetKey)).add(BigInteger.ONE));
		else
	...
	for (int i = 0; i < histDim; i++) {
	    distSum[i] /= counter;
	    distVar[i] = Math.sqrt(distVar[i] / counter
		    - Math.pow(distSum[i], 2));
	...
	if (print) {
		...
		System.out.println("pivot " + pivot[fixedMapping[i]] + " = ("
			+ lowerBound[fixedMapping[i]] + " ~ "
			+ upperBound[fixedMapping[i]] + ") ");
	...
	for (int i = 0; i < histDim; i++) {
		...
		System.out.println("pivot " + pivot[histMapping[i]] + ": ["
			...
			+ upperBound[histMapping[i]] + ", " + distVar[i] + ", "
			+ histEntropy[i] + " ]:  " + width[histMapping[i]]);
	...
	if (print) {
	    System.out.println("Sum of variance, entropy, on all dimensions ="
		    + distVar[0] + ", e: " + histEntropy[0]);

	    System.out.println("map size  = " + map.size()
		    + ",   dataset size = " + (int) sum + ",  pivot number = "
		    + pivotNumber + ", max bin size = " + max);
	}
	...
	if (print)
	    System.out.println("Std. dev. among non-empty bins = "
		    ...
		    + ", 2-raw moment = " + (var / counter) / counter
		    + ", entropy = " + (Math.log(counter) - entropy / counter));

	result.append(" " + (Math.log(counter) - entropy / counter) + " "
		+ histEntropy[0] + " " + (var / counter) / counter + " "
	...
	while (p.hasNext()) {
	    ...
	    key = (String) entry.getKey();
	    if (histDim == 1) {
		histogram1D[(int) key.charAt(0)] = ((BigInteger) entry
			.getValue()).intValue();
	    } else if (histDim == 2) {
		...
		/*
		 * System.out.print( "("); for (int i=0; i< histDim; i++)
		 * System.out.print( (int) key.charAt(i) + ", ");
		 * System.out.println( "): " + entry.getValue());
		 */
	    }
    ...
    public static double allPairs(String[] args, boolean print)
	    throws Exception {
	String fileName = args[0];
	...
	for (int i = 0; i < dim - 1; i++) {
		...
		tempString = continuousPivotSpaceHistogram(parameter.toString()
			.split(" "), print);
	...
	double[][] hist = oneDHistogram(3.1, 1, new double[] { 1, 2, 3, 4, 5,
		12, 30 });
	...
	/*
	 * char [] s = new char[5]; s[0] = (char) 0; s[1] = (char) 1; s[2] =
	 * (char) 2; s[3] = (char) 95; s[4] = (char) 102; String ss = new
	 * String(s); for (int i=0; i< ss.length(); i++) System.out.println(
	 * (int) ss.charAt(i) ); System.out.println(ss + ss.length());
	 */
    }
}