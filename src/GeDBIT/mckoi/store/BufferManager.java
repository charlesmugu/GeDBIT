 ...
 * implementstions.  This object manages memory usage of the children buffers
 * and implements a caching strategy for purging old pages from memory.
 *
...
public final class BufferManager {
  ...
  private long cache_miss_count;
	 ...
	 */
	public BufferManager(String paging_implementation, int max_pages, int page_size) {
	 ...
	 */
	public synchronized FileBufferAccessor createBufferedAccessor(RandomAccessFile file, String mode) throws IOException {
		...
		if (paging_implementation.equals("Java IO")) {
			...
			throw new RuntimeException("No not know how to create a buffered accessor for paging " + "implementation: "
					+ paging_implementation);
		}
	}
    ...
    synchronized void pageCreated(PageBuffer buffer) throws IOException {
		...
		if (current_page_count > max_pages) {
			...
			/*
			if (page_with_root == null) {
				int size = page_list.size();
				PageBuffer page;
				PageBuffer root_page = (PageBuffer) page_list.get(0);
				int root_page_location = 0;
				for (int i = 1; i < size; i++) {
					page = (PageBuffer) page_list.get(i);
					if (page.getPageNumber() > root_page.getPageNumber()) {
						root_page = page;
						root_page_location = i;
					}
				}
				//page_with_root =
				// (PageBuffer)page_list.remove(root_page_location);
				page_with_root = (PageBuffer) page_list.get(root_page_location);
				System.out.println("number of pages:" + page_with_root.getPageNumber());
			}
			*/
	...
	private final float pageEnumValue(PageBuffer page) {
		...
		// cache forever.
		return (1f / Math.min(page.getAccessCounter(), 10000)) * (current_T - page.getT());
	}
	...
	public void printStatistics() {
		...
		System.out.println("Total Write Time:"+(total_write_time/1000)+" seconds.");
		System.out.println("Avg Write Time:"+(avg_write_time/1000)+" seconds.");
		System.out.println("Total Read Time:"+(total_read_time/1000)+" seconds.");
		System.out.println("Avg Read Time:"+(avg_read_time/1000)+" seconds.");
		System.out.println("Highest Read Time:"+(highest_read_time/1000)+" seconds.");
		System.out.println("Lowest Read Time:"+(lowest_read_time/1000)+" seconds.");
		System.out.println("Total I/O Time:"+(total_i_o/1000)+" seconds.");		
	}
}
