...
//    System.out.println("- Removing from index " + bin_chain_index);
  ...
  public void setFixedArea(byte[] buf, int off, int len) throws IOException {
    if (len > 64) {
      throw new IOException("Fixed area size can be no greater than 64 bytes.");
    }
  ...
  public void getFixedArea(byte[] buf, int off, int len) throws IOException {
    if (len > 64) {
      throw new IOException("Fixed area size is no greater than 64 bytes.");
    }
...
class StoreIterator implements Iterator
  {
      ...
      private boolean obsolete;  // whether current position is already returned by next()
      ...
      {
          if (!hasNext() )
              throw new java.util.NoSuchElementException( "Store has no next !");
      ...
      {
          throw new UnsupportedOperationException( "remove() in StoreIterator is not supported yet!");
      }
