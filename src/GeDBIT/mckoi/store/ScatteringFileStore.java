 ...
 * files in the filesystem. If the amount of data stored in one store exceeds a
 * given threshold the data grows into a new file.
 * <p>
...
public final class ScatteringFileStore extends AbstractStore {
     ...
     */
    public ScatteringFileStore(File path, String file_name, String first_ext,
	    long max_slice_size, BufferManager buffer_manager, boolean read_only) {
	super(read_only);
	this.path = path;
    ...
    /**
     * Given a file, this will convert to a scattering file store with files no
     * larger than the maximum slice size.
     */
	...
	slice.buffered_file = buffer_manager.createBufferedAccessor(slice.file,
		mode);
	...
	if (open_existing) {
		...
		slice.buffered_file = buffer_manager.createBufferedAccessor(
			slice.file, mode);
		slice.length = slice.file.length();
		slice_list.add(slice);
	...
	for (int i = 0; i < sz; ++i) {
		...
		throw new IOException(
			"Error: file size tally is incorrect for file: "
				+ slice_part.name + "val = "
				+ slice_part.length + " act = "
				+ slice_part.file.length());
	    }
	}
    ...
    protected synchronized int readByteArrayFrom(long position, byte[] buf,
	...
	while (len > 0) {
	    ...
	    FileSlice slice = (FileSlice) slice_list.get(file_i);
	    int read_count = slice.buffered_file.readByteArray(file_p, buf,
		    off, file_len);
    ...
    protected synchronized void writeByteArrayTo(long position, byte[] buf,
	    int off, int len) throws IOException {
	...
	while (total_size_to_grow > 0) {
	    ...
	    // Synchronize the file change. XP appears to defer a file size
	    // change
    ...
    /**
     * Prints statistics about the buffer manager. Statistics include number of
     * hits, misses, amount of time spent in reading data from disk, amount of
     * time spent in writing data to disk, etc. Added by Willard, 2005.05.24
     */
