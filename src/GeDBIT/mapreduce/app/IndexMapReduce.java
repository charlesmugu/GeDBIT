...
public class IndexMapReduce
{
    ...
    public static class Map extends Mapper<Text, BytesWritable, IntWritable, Text>
    {
        public void map(Text key, BytesWritable value, Context context) throws IOException, InterruptedException
        {
            ...
            for (int size = initialSize, i = 0; (size <= initialSize) & (i < indexNum); size += stepSize, i++)
            {
                ...
                try
                {
                    table = getTable(dataType, fileName, currentIndexPrefix, size, frag, dim, data);
                }
                catch (IOException e)
                {
                    e.printStackTrace();
                ...
                QueryVPIndexMR evaluator = new QueryVPIndexMR(table.getIndex(), queryTable, minRadius, maxRadius, step, verify
            ...
            {
                //context.write(null, new IntWritable(resultList.get(i).getRowID()));
                StringBuffer sb = new StringBuffer();
                sb.append("radius: " + queryRadiusData.get(radiusNum).radius + "  data: ");
    ...
    {  
        ...
        for (int i = 0; i < args.length; i = i + 2)
        {
            ...
            else if (args[i].equalsIgnoreCase("-psm"))
            {
                ...
                verifyArgs = (Integer.parseInt(args[i + 1]) == 1) ? true : false;
        ...
        if (psmNameArgs.equalsIgnoreCase("incremental"))
            psmArgs = new GeDBIT.index.algorithms.IncrementalSelection(setAArgs, setNArgs);
            ...
            psmArgs = new GeDBIT.index.algorithms.SelectionOnFFT(fftScaleArgs, testKindArgs, yMethodArgs, selectAlgorithmArgs);
        ...
        run(queryFilePathArgs, queryResultPathArgs, dpmArgs, dataTypeArgs, dimArgs, psmArgs, pNumArgs, sfArgs, mlsArgs, pathLengthArgs, 
                bucketArgs, maxRArgs, fftScaleArgs, lastQueryArgs, minRadiusArgs, maxRadiusArgs, stepArgs, verifyArgs, firstQueryArgs); 
    }
    
    public static void run(String queryFilePathArgs,String queryResultPathArgs, PartitionMethod dpmArgs, String dataTypeArgs, int dimArgs, 
            PivotSelectionMethod psmArgs, int pNumArgs, int sfArgs, int mlsArgs,int pathLengthArgs,boolean bucketArgs,double maxRArgs,int fftScaleArgs,
            int lastQueryArgs,double minRadiusArgs,double maxRadiusArgs,double stepArgs,boolean verifyArgs,int firstQueryArgs) throws IOException, InterruptedException, ClassNotFoundException
    {        
            ...
            {
                System.err.println("Usage: -q queryFilePath -o queryResultPath and other parameters");
                System.exit(2);
            }
    ...
    private static TableMR getTable(String dataType, String fileName, String indexPrefix, int maxDataSize, int fragLength, int dim, double[][] data) throws IOException
    {
        TableMR table = null;
        ...
        {
            table = new DoubleVectorTableMR(fileName, indexPrefix, maxDataSize, dim, data);
        }
        /*else if (dataType.equalsIgnoreCase("image"))
        {
