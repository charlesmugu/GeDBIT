...
public class QueryVPIndexMR
{
    ...
    // objects.
    int[]                   minCenterDistNum, minCenterDistNumID, maxCenterDistNum, maxCenterDistNumID;
    ...
    double[] resultInternalNode;   //number of internal nodes containing at least one query result;
    double[] resultLeafNode;   //number of leaf nodes containing at least one query result;
    double[] resultNode;   //number of nodes containing at least one query result;
//    double[] pivotDistNum;
    ...
    int[]                   minInternalVisited, minInternalVisitedID, maxInternalVisited, maxInternalVisitedID;
    ...
    /*public static void main(String[] args)
    {
        ...
        if ((firstQuery < 0) || (lastQuery < 0) || (lastQuery < firstQuery))
            throw new IllegalArgumentException("Invalid first query index or last query index!");

        if ((minRadius < 0) || (maxRadius < 0) || (maxRadius < minRadius) || (step <= 0))
            throw new IllegalArgumentException("Invalid min radius, max radius, or radius increasement unit!");

        // load index from file
        TableMR dataTable = TableManager.getTableManager(indexName).getTable(indexName);
        Index index;
        if (dataTable != null)
        ...
        try
        {
            if (dataType.equalsIgnoreCase("protein"))
                ...
                queryTable = new DoubleVectorTableMR(queryFileName, "", lastQuery, dim, data);
        ...
        // evaluate
        QueryVPIndexMR evaluator = new QueryVPIndexMR(index, queryTable, minRadius, maxRadius, step, verify
...
//    public QueryVPIndex(Index index, Table query, double minRadius, double maxRadius, double step, boolean verify, Level debug, int pathLength,
//            int frag, String resultsFileName)
//    {
//        this(index, query, minRadius, maxRadius)
//    }
    
    public QueryVPIndexMR(Index index, TableMR query, double minRadius, double maxRadius, double step, boolean verify, Level debug, int pathLength,
...
//        pivotDistNum  = new double[numRun];
     ...
     */
    public void evaluate(List<IndexObject> resultList, List<radiusData> queryRadiusData)
    {
        ...
        if (outputToFile && !outputFile.equals(""))
        {
            ...
            try
            {
                ...
                for (int i = 0; i < numRun; i++)
                {
                    ...
                    outputStream.println(indexSize + ", " + fmt.format((minRadius + i * step)) + ", #dist:[, " + distNum[i] + " ,(, "
                            + (minDistNum[i]) + " ,:, " + (minDistNumID[i]) + " ,) (, " + (maxDistNum[i]) + " ,:, " + (maxDistNumID[i]) + " ,)] "
                            + "{ [, " + (centerDistNum[i]) + " ,(, " + (minCenterDistNum[i]) + " ,:, " + (minCenterDistNumID[i]) + " ,) (, "
                            + (maxCenterDistNum[i]) + " ,:, " + (maxCenterDistNumID[i]) + " ,)] " + "[, " + (dataDistNum[i]) + " ,(, "
                            + (minDataDistNum[i]) + " ,:, " + (minDataDistNumID[i]) + " ,) (, " + (maxDataDistNum[i]) + " ,:, "
                            + (maxDataDistNumID[i]) + " ,)]}");

                    outputStream.println(indexSize + ", " + (minRadius + i * step) + ", #node: [, " + nodeVisited[i] + " ,(, " + minNodeVisited[i]
                            + " ,:, " + minNodeVisitedID[i] + " ,) (, " + maxNodeVisited[i] + " ,:, " + maxNodeVisitedID[i] + " ,)] " + "{ [, "
                            + internalVisited[i] + " ,(, " + minInternalVisited[i] + " ,:, " + minInternalVisitedID[i] + " ,) (, "
                            + maxInternalVisited[i] + " ,:, " + maxInternalVisitedID[i] + " ,)]  " + "[, " + leafVisited[i] + " ,(, "
                            + minLeafVisited[i] + " ,:, " + minLeafVisitedID[i] + " ,) (, " + maxLeafVisited[i] + " ,:, " + maxLeafVisitedID[i]
                            + " ,) ]}");

                    outputStream.println(indexSize + ", " + (minRadius + i * step) + ", time: [, " + time[i] + " ,(, " + minTime[i] + " ,:, "
                            + minTimeID[i] + " ,) (, " + maxTime[i] + " ,:, " + maxTimeID[i] + " ,)]");

                    outputStream.println(indexSize + ", " + (minRadius + i * step) + ", #result: [, " + result[i] + " ,(, " + minResult[i] + " ,:, "
                            + minResultID[i] + " ,)(, " + maxResult[i] + " ,:, " + maxResultID[i] + " ,)]");

                    outputStream.print(indexSize + ", " + (minRadius + i * step) + ", #layer node: [ ");

                    for (int j = 0; j < layerNode[i].length; j++)
                        outputStream.print("( " + j + ":, " + layerNodeVisited[i][j] + " / " + layerNode[i][j] + "), ");

                    outputStream.println("]");

                    outputStream.print(indexSize + ", " + (minRadius + i * step) + ", #layer data object: [ ");
                    for (int j = 0; j < layerData[i].length; j++)
                        outputStream.print("(" + j + ":, " + layerDataVisited[i][j] + " / " + layerData[i][j] + "), ");

                    outputStream.println("]");

                    outputStream.print(indexSize + ", " + (minRadius + i * step) + ", query-center dist: [ ");
                    for (int j = 0; j < queryCenterDistance[i].length; j++)
                    {
                        outputStream.print(j + ": (,");
                        for (int k = 0; k < queryCenterDistance[i][j].length; k++)
                            outputStream.print(fmt.format(queryCenterDistance[i][j][k]) + ", ");
                        outputStream.print(") ");
                    }
            ...
            catch (IOException e)
            {
                ...
                e.printStackTrace();
                System.out.println("\nError: Problem creating output file \"" + outputFile + "\". Program aborted.");
                System.exit(0);
            }

            System.out.println("Finished writing statistics file \"" + outputFile + "\" to disk.");

        }
        else
        {
            System.out.println("index size, radius, #distance calculation, #node visited, search time, #result,,#result node, #result internal node, #result leaf node, internalPruned, leafPruned, pivotDistNum, dataDistNum, pivotAsResult, ResultWithoutDist, internalWithoutDist, leafWithoutDist");
            for (int i = 0; i < numRun; i++)
            {
                System.out.println(forPrint + indexSize + ", " + fmt.format((minRadius + i * step)) + ", " + distNum[i] + ", " + nodeVisited[i] + ", " + time[i]
                        + ", " + result[i] + ", " + resultNode[i] + ", " + resultInternalNode[i] + ", " + resultLeafNode[i] + ", " 
                        + internalPruned[i] + ", " + leafPruned[i] + ", " + centerDistNum[i] + ", " + dataDistNum[i] + ", " + pivotAsResult[i] + ", " + resultWithoutDistance[i] + ", " + internalWithoutDist[i] + ", " + leafWithoutDist[i]);
            }
        } // end of else
...
//        pivotDistNum[offset] = 0;
        ...
        if (resultsFileName != null)
        {
            try
            {
                resultsFile = new PrintWriter(new BufferedWriter(new FileWriter(resultsFileName)));
            }
            catch (IOException e)
            {
                e.printStackTrace();
        ...
        List query2 = allQuery.subList((firstQuery < 0)?0: firstQuery, (lastQuery >allQuery.size())?allQuery.size():lastQuery);
        Iterator p = query2.iterator();
        int queryCounter = -1;
        while (p.hasNext())
        {
            queryCounter++;
            RangeQuery q = new RangeQuery((IndexObject) p.next(), radius, pathLength);
            if (resultsFile != null)
            {
            ...
            {
                IndexObject iObject = ((DoubleIndexObjectPair) cursor.next()).getObject();
                if (resultsFile != null)
                {
     ...
     * p.hasNext() ) query.add ( ( (Pair) p.next() ).first() ); return new Pair(
     * metric, query); } static public Pair loadVectorQuery(String fileName,
     * final int first, final int last) { Metric metric =
     ...
     * query = new ArrayList( temp.size() ); Iterator p = temp.iterator(); while (
     * p.hasNext() ) query.add ( ( (Pair) p.next() ).first() ); return new Pair(
     * metric, query); } static public Pair loadHammingQuery(String fileName,
     * final int first, final int last) { Metric metric = new
