...
public enum PartitionMethods implements PartitionMethod {
	...
	PartitionResults partition(Metric metric, IndexObject[] pivots,
		...
		double temp;
		while (head <= tail) {
		    while ((head < size) && (distance[1][head] <= 0))
			head++;
		    while ((tail >= 0) && (distance[1][tail] > 0))
			tail--;
		    if (head <= tail) {
	...
	private double[] bucketInitialClustering(double[] distance,
	    ...
	    final int bucketNumber = Math.max(
		    Math.min((last - first + 1) / 10, 50 * SVF), SVF);
    ...
    /*
     * constant used to hold input parameters Added by Kewei Ma
     */
