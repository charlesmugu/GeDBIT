...
public class Gauss
{
    ...
    public Gauss(double[][] m_x, int r)
    {
        row = r;
        col = row+1;
        gauss_matrix = new double[row][row+1];
        for(int i=0; i<row; i++)
        //for every rows
        {
            double sum = 0;
    ...
    */
    ...
    public void calSolution()
    {
        ...
        for(int i = row-1; i>=0; i--)
        {
            double temp = 0;
            for(int j=i+1; j<row; j++)
                temp+=gauss_matrix[i][j]*solution_double_vector[j];
            temp -= gauss_matrix[i][col-1];
            solution_double_vector[i] = (-temp)/gauss_matrix[i][i];
        }
        ...
        for(int i=0; i<row; i++)
        {
            ...
            Double temp_double = solution_double_vector[i];
            solution_float_vector[i] = temp_double.floatValue();
            Float temp_float = solution_float_vector[i];
            solution_int_vector[i] = temp_float.longValue();
        }
    }
    */
