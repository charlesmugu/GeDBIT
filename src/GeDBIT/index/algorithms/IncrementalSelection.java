...
public class IncrementalSelection implements PivotSelectionMethod, java.io.Serializable
{
     ...
     * @return
     */
    public int[] selectPivots(Metric metric, List<? extends IndexObject> data, int first, int dataSize,
            final int numPivots)
    {
        ...
        for (int k=0; k< NP; k++)
        {
            ...
            for (int i=0; i<N; i++)
            {
                ...
                for (int j=0; j<A; j++)
                {
                    ND[i][j] = Math.max(D[j], Math.abs(   metric.getDistance(data.get(setA[0][j]), data.get(setN[i])) 
                                                        - metric.getDistance(data.get(setA[1][j]), data.get(setN[i])) )) ;
                    sum += ND[i][j];
                }
