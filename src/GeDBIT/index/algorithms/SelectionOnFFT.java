...
public class SelectionOnFFT implements PivotSelectionMethod, java.io.Serializable
{
    /*
            这个就是照着PCAOnFFT做的界面。
            传的参数要完全符合预先设定的值，包括大小写，否则会出错
            我感觉这样传参会比较麻烦，先这样吧，再想想有没有什么更好的方法
            我给这个类加了3个变量：testKind 用什么标准（现在用的是F-test）
    yMethod y怎么求得
    selectAlgorithm 用的是forward还是backward
    ...
    */
    ...
    public int[] selectPivots(Metric metric, List<? extends IndexObject> data, int numPivots){
        ...
        */
        ...
        for(int col = 0; col < fftResult.length; col++)
            for(int row = 0; row < dataSize; row++)
                matrix[row][col+1] = metric.getDistance(data.get(row), data.get(fftResult[col]));
        /*
        if(dataSize == fftResult.length)
        {
            System.out.println("here comes matrix:");
            for(int i=0; i<dataSize; i++)
            {
                for(int j=1; j<=fftResult.length; j++)
        ...
        */
        ...
        if(selectAlgorithm.equalsIgnoreCase("forward"))
            ...
            throw new IllegalArgumentException("Invalid option " + selectAlgorithm);
    ...
    double[][] getYStand(double[][] matrix, int n, int p){
        ...
        //double sum = 0, sSum = 0;
        for(int i=0; i<n; i++){
            double sum = 0, sSum = 0, avg = 0;
            //计算每行的标准差
            for(int j=1; j<=p; j++){
                sum += matrix[i][j];
                //和的平方
                sSum += matrix[i][j]*matrix[i][j];
            }
            //均值的平方
            avg = sum/p;
            avg = avg * avg;
