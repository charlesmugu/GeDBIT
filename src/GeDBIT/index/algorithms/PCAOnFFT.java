...
public class PCAOnFFT implements PivotSelectionMethod, java.io.Serializable {
	...
	/*
	 * if (dataSize < 10) { System.out.println("size = " + dataSize +
	 * ", numPivots = " + numPivots); for (int i=0;i<dataSize; i++) { double
	 * []v = ( (GeDBIT.type.DoubleVector)data.get(i)).getData(); for (int
	 * j=0; j<v.length; j++) System.out.print(v[j] + "   ");
	...
	int[] fftResult = PivotSelectionMethods.FFT.selectPivots(metric, data,
		numPivots * FFTScale);
	/*
	 * if (dataSize <10) { System.out.print("fft result: "); for (int j=0;
	 * j<fftResult.length; j++) System.out.print(fftResult[j] + "   ");
	 * System.out.println(); }
	 */
	...
	DoubleMatrix2D matrix = LargeDenseDoubleMatrix2D.createDoubleMatrix2D(
		dataSize, fftResult.length);
	...
	try {
	    ...
	    System.out.println("Exception!  data size =" + dataSize
		    + ", num pivots = " + numPivots);
	    e.printStackTrace();
