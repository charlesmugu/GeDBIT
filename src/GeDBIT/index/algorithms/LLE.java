...
public class LLE {
	...
	final int d = numP;    //number of pivots, also the output dimension	
		...
		for (int row = 0; row < rowNumber; row++) {
			...
			for (int i = 0; i < k; i++)
				weightMatrix
						.setQuick(nnbIndexsTemp[i + 1], row, w.getQuick(i));
    ...
    public static int[] selectByCov(DoubleMatrix2D matrix,
	    ...
	    for(int i = 0;i<evMatrix.columns();i++){
	        ...
	        for(int j=0;j<matrix.columns();j++){
	            ...
	            double m,n=0.0;
	            m = Transform.mult(Transform.minus(evMatrix.viewColumn(i).copy(), evAvg), Transform.minus(matrix.viewColumn(j).copy(), matAvg)).zSum();
	            n = Math.sqrt(Transform.pow(Transform.minus(evMatrix.viewColumn(i).copy(), evAvg), 2).zSum())*Math.sqrt(Transform.pow(Transform.minus(matrix.viewColumn(j).copy(), matAvg), 2).zSum());
	            r[j] = m/n;
	        }
	...
	/**
	 * Select some of the axes as pivots according to the angle between them , no duplicate axes will be selected.  
	 * 
	 * @param the original distance matrix
	 * @param matrix of eigenvector from runLLE
	 * @return
	 */
	...
	/**
	 * modified from GeDBIT.index.algorithms.PCA.pairWiseDistance
	 ...
	 */
	   public static DoubleMatrix2D pairWiseDistance(Metric metric, List<? extends IndexObject> data)
	    {
	        DoubleMatrix2D distance = new DenseDoubleMatrix2D(data.size(),data.size());
