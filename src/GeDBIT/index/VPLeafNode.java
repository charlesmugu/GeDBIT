...
public class VPLeafNode extends LeafNode {
     ...
     * @param pathDistance
     *        distances from each distinct data point (row) to each pivot(column) on the path from
     *        the root to this node.
     */
    public VPLeafNode(IndexObject[] pivots, IndexObject[] children, int size, double[][] distance,
            double[][] pathDistance) {
        ...
        if (pathDistance == null)
            throw new IllegalArgumentException("pathDistance array cannot be null!");
    ...
    /**
     * @param childIndex
     * @return the path distance list, consisting of the distances from a child to pivots on the
