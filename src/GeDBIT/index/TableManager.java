...
public class TableManager implements Serializable
{
    ...
    public static TableManager getTableManager(String prefix)
    {
        if (tableManager == null)
        {
            if (new File(prefix + "-GeDBIT.db").exists())
            {
                ...
                try
                {
                    objectStream = new ObjectInputStream(new FileInputStream(prefix + "-GeDBIT.db"));
    ...
    {
        oiom = new MckoiObjectIOManager(this.indexPrefix + "-GeDBIT", "000", 1024 * 1024 * 1024, "Java IO", 4, 128 * 1024, readOnly);
    }
    ...
    protected void openOIOM()
    {
        try
        {
            if (!oiom.open())
            {
                throw new Error("Cannot open store for TableManager" + this.indexPrefix + "-GeDBIT.000");
            }
    ...
    public Table getTable(int tableIndex)
    {
        if (!tables.containsKey(tableIndex))
        {
            try
            {
                tables.put(tableIndex, (Table) oiom.readObject(oiomLocations.get(tableIndex)));
            }
            catch (IOException e)
            {
                e.printStackTrace();
    ...
    public void close()
    {
        try
        {
            ObjectOutputStream out = new ObjectOutputStream(new FileOutputStream(this.indexPrefix + "-GeDBIT.db"));
