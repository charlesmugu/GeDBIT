...
public class VPRangeCursor extends Cursor
{
    ...
    int                 distNum, pivotDistNum, dataDistNum; //number of distance calculation, calculations with pivots, data with points
    int resultNode, resultInternalNode, resultLeafNode;  //number of nodes containing at least one query result.
    int internalPruned, leafPruned;  //number of points pruned in internal nodes, leaf nodes.
    ...
    public VPRangeCursor(RangeQuery query, ObjectIOManager[] oioms, Metric metric, long rootAddress)
    {
    ...
    protected void visit(QueryTask t)
    {
        ...
        if (!(t instanceof VPRangeCursor.RangeQueryTask))
            throw new UnsupportedOperationException("Only RangeQueryTask is supported by VPRangeCursor, not "
                    + t.getClass());
        ...
        if (node instanceof VPInternalNode)
            ...
            throw new UnsupportedOperationException(
                    "only VPInternalNode or VPLeafNode is supported by VPRangerCursor, not " + node.getClass());
    }
    ...
    private void visitInternalNode(VPInternalNode node, double[] distList)
    {
        ...
        // ArrayList toSearch = new ArrayList(); //the children to further search
        ...
        for (int pivot = 0; pivot < numPivot; ++pivot)
        {
            ...
            {
                this.result.add(new DoubleIndexObjectPair(queryPivotDistance[pivot], node.getPivot(pivot)));
                
                //statistics, number of (internal) nodes containing at least one query result
                if (!gotResult)
                {
                    gotResult = true;
                    this.resultNode ++;
        ...
        // hack: if node has cght partition, transform distances to d1+d2 an d1-d2
        if (node.GHTDegree < 0)
        {
            ...
            queryPivotDistance[0] = queryPivotDistance[0] + queryPivotDistance[1];
            queryPivotDistance[1] = queryPivotDistance[0] - queryPivotDistance[1]*2;
        }
            ...
            for (int i = 0; i < numChild; ++i)
            {
                ...
                for (int j = 0; j < numPivot && !done; ++j)
                {
                            ...
                            && (range[1][j] + queryPivotDistance[j] <= r))
                    {
                        ...
                        resultNode += getNodeNumber(this.oiom, node
                                .getChildAddress(i));
                        resultInternalNode += getInternalNodeNumber(this.oiom,
                                node.getChildAddress(i));
                        resultLeafNode += getLeafNodeNumber(this.oiom, node
                                .getChildAddress(i));
                    ...
                    if (!done
                            && ((queryPivotDistance[j] + ((node.GHTDegree < 0) ? 2 * r : r) < range[0][j])
                             || (queryPivotDistance[j] - ((node.GHTDegree < 0) ? 2 * r : r) > range[1][j])))
                    {
                        done = true;
        ...
        else  //ght search
        {
            ...
            if (queryPivotDistance[1] <= -2 * r)
            {
                ...
                {
                    this.nodePrunedByPivotDist++;
                    int temp = getAllPoints(this.oiom, node.getChildAddress(1)).size();
                    this.pointPrunedByPivotDist += temp;
                    this.internalPruned += temp;
                }
                if (Debug.debug) // operation of statistics
                {
                    logger.finest("(" + queryPivotDistance[1] + "<= -2*" + r + ") : pruned], ");
                }
            ...
            else
            {
                // TODO
                if (oldDistListLength < distList.length)
                    this.task.add(new RangeQueryTask(node.getChildAddress(1), (double[]) distList.clone()));
                else
                    task.add(new RangeQueryTask(node.getChildAddress(1), distList));
            ...
            if (queryPivotDistance[1] > 2 * r)
            {
                ...
                {
                    this.nodePrunedByPivotDist++;
                    int temp = getAllPoints(this.oiom, node.getChildAddress(0)).size();
                    this.pointPrunedByPivotDist += temp;
                    this.internalPruned += temp;
                }
                if (Debug.debug) // operation of statistics
                {
                    logger.finest("(" + queryPivotDistance[0] + "> 2*" + r + ") : pruned], ");
                }
            ...
            else
            {
                // TODO
                if (oldDistListLength < distList.length)
                    this.task.add(new RangeQueryTask(node.getChildAddress(0), (double[]) distList.clone()));
                else
                    task.add(new RangeQueryTask(node.getChildAddress(0), distList));
    ...
    private void visitRootInternalNode(VPInternalNode node, double[] distList)
    {
            ...
            for (int i = 0; i < numChild; ++i)
            {
                ...
                for (int j = 0; j < numPivot && !done; ++j)
                {
                            ...
                            && (range[1][j] + queryPivotDistance[j] <= r))
                    {
                        ...
                        List<IndexObject> allResult = getAllPoints(this.oioms[i + 1],
                                node.getChildAddress(i));
                ...
                if (!done)
                {
                    if (oldDistListLength < distList.length)
                        ...
                        this.task.add(new RangeQueryTask(i, node.getChildAddress(i),
                                distList));
                }
        ...
        else  //ght search
        {
            ...
            {
                if (oldDistListLength < distList.length)
                    this.task.add(new RangeQueryTask(1, node.getChildAddress(1), (double[]) distList.clone()));
                else
                    this.task.add(new RangeQueryTask(1, node.getChildAddress(1), distList));
            }
            ...
            {
                if (oldDistListLength < distList.length)
                    this.task.add(new RangeQueryTask(0, node.getChildAddress(0), (double[]) distList.clone()));
                else
                    this.task.add(new RangeQueryTask(0, node.getChildAddress(0), distList));
            }
        }
    }
    ...
    private void visitLeafNode(VPLeafNode node, double[] distList)
    {
        ...
        // int doneNum = 0; //number of distinct data points that are already pruned.
        ...
        for (int child = 0; child < distinctSize; child++)
        {
            ...
            for (int pivot = 0; pivot < distList.length && pivot < pathDist.length; pivot++)
            {
                ...
                if (pathDist[pivot] + distList[pivot] <= r)
                {
                    ...
                    this.result.add(new DoubleIndexObjectPair(-1, node.getChild(child)));
            ...
            if (distance <= r)
            {
                ...
                this.result.add(new DoubleIndexObjectPair(distance, node.getChild(child)));
    ...
    public List<IndexObject> searchRangeQuery(double[] distList)
    {
        ...
        while (this.hasNext())
        {
            IndexObject iobject = ((DoubleIndexObjectPair) this.next()).getObject();
    ...
    void distributeTasks()
    {
        ...
        if(!this.result.isEmpty())
        {
            ...
            if(this.task.isEmpty()) {
                ...
                Task newTask = new GlobalIndexPrintTask(0, forprint.toString(), latch);
                ...
                WorkThread thread = (WorkThread) threadspool.get(queryTask.childNum % threadspool.size());
                Task newTask = new GlobalIndexWorkTask(thread.getThreadID(), this.metric, queryTask.nodeAddress, queryTask.distList, this.id, this.r, forprint.toString(), latch);
                LinkedList<Task> queue = thread.getQueue();
                synchronized (queue)
                {
                    queue.addLast(newTask);
        ...
        while(!this.task.isEmpty())
        {
            queryTask = this.task.removeFirst();
            WorkThread thread = (WorkThread) threadspool.get(queryTask.childNum % threadspool.size());
            Task newTask = new GlobalIndexWorkTask(thread.getThreadID(), this.metric, queryTask.nodeAddress, queryTask.distList, this.id, this.r, "", latch);
            LinkedList<Task> queue = thread.getQueue();
            synchronized (queue)
            {
                queue.addLast(newTask);
