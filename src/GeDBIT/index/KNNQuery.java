...
/**
 * This class contains implmentation for K nearest neighbor search (KNN) 
 * and approximate K nearest neighbor search (AKNN) queries. 
 * A KNN query is in the form of (q, k). 
 *  Given (q, k), the search returns the k closest objects to query object q.
 * An aproximate KNN query is in the form of (q, k, r, sp), 
 * where r is a limiting radius and sp is the approximation policy.
 * The search proceed accroding to the value of r and sp. 
 * 1) When both r and sp is omit, the search proceeds as KNN search
 * 2) When r is given and sp is omit or sp is the value of KNNQuery.RADIUSLIMIRED,
 *     the search returns upto k objects that are the closest ones to query 
 *     q within radius r.
 ...
 *    The results of search with larger sp value have better accuracy than those of 
 *    search with lower sp value and take longer to compute.   
...
public class KNNQuery implements Query 
{
    ...
    public KNNQuery (IndexObject center, int k, double radius, int sp, int listSize) 
    {
        ...
        if (listSize < 0)
            throw new IllegalArgumentException ("max distance list size < 0: " + listSize) ;
