...
public class DoubleVectorTableMR extends TableMR {
     ...
     * @throws IOException
     */
    public DoubleVectorTableMR(String fileName, String indexPrefix, int size,
	    int dimNum, double[][] data) throws IOException {
     ...
     */
    public DoubleVectorTableMR(String fileName, String indexPrefix, int size,
	    int dimNum, Metric metric, double[][] data) throws IOException {
     ...
     * @throws IOException
     */
    void loadData(/* BufferedReader reader, */int maxSize, int dimNum,
	 ...
	 * if (dim < dimNum) dimNum = dim; // size =
	 * java.lang.Integer.parseInt(metaData[1]); // total number of // data
	 * 
	 * if (Debug.debug) { Logger.getLogger("GeDBIT.index").info("dim: " +
	 * dim); Logger.getLogger("GeDBIT.index").info("size: " + maxSize); }
	 * 
	 * int numData = 0;
	 */
	...
	/*
	 * line = reader.readLine(); if (line != null) line = line.trim();
	 * 
	 * while (line != null && numData < maxSize) { String[] row =
	 * line.split("[ \t]+");
	 * 
	 * if (Debug.debug) { for (int i = 0; i < row.length; i++)
	 * Logger.getLogger("GeDBIT.index").finer("row[" + i + "]: " + row[i]);
	 * }
	 * 
	 * for (int i = 0; i < dimNum; i++) data[i] =
	 * java.lang.Double.parseDouble(row[i]);
	 * 
	 * // System.out.println(new DoubleVector(new Integer(numData), data));
	 * 
	 * originalRowIDsArrayList.add(numData, numData); doubleVectors.add(new
	 * DoubleVector(this, numData, data));
	 * 
	 * line = reader.readLine(); if (line != null) line = line.trim();
	 * 
	 * numData++; }
