...
public class ImageTable extends Table
{
    ...
    private void loadData(BufferedReader reader, int size)
    {
        ...
        {
            e.printStackTrace();
            throw new java.lang.IllegalStateException("Error occured when reading Image features file: " + reader);
        }
        images.trimToSize();
        data = images;
    }

    private void loadData(BufferedReader reader, BufferedReader readerMaxInfo, int size)
    {
        String line;
        String lineMaxInfo;
        int count = 0;
        ...
        try
        {
            ...
            while (line != null && lineMaxInfo != null && count <= size)
            {
                ...
                {
                    maxDist[i] = java.lang.Double.parseDouble(lineMaxInfoSegments[i]);
                }
