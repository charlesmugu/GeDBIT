		...
		<div id="dna_search_summary_details">
			...
			<div id="dna_search_summary_details_img">
				<ul class="dna_img_item" id="sources">
					 <input type="hidden" value="<s:property value="prostr" />" id="usrsub"/>
				</div>
			</div>
			-->
		</div>
...
<div id="dna_search_results">
  ...
  <input type="hidden" value="<s:property value='#st.index' />" id="jscount<s:property value='#st.index'/>" class="jc"/>
			<div class="dna_search_results_item clearfix">
				<div class="dna_search_results_item_details">
					From Protein: <a href="#"> <s:property value="key" /></a><br/>
					Similar part:<p style="font-size:14px;font-weight: normal;"><s:property value="value"  escape="false" /></p>
				</div>
				<!--  /////DNA 图片
				...
				<s:if test="%{page>0}">
					<a href="?prostr=<s:property value='prostr'/>&page=<s:property value='%{page-1}'/>" id= >上一页</a>
				</s:if>
				<s:if test="%{page>0}">
				...
				<s:if test="%{page<allpage}">
					<a href="?prostr=<s:property value='prostr'/>&page=<s:property value='%{page+1}'/>" id= >下一页</a>	
				</s:if>
			</div>
		</div>
	</div>
